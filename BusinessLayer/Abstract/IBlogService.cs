...
public interface IBlogService
{
    ...
    Task<IDataResult<IPagedList<Blog>>> GetListWithCategoryAsync(bool? status = null, int pageNumber
        = 10, int pageSize = 10);
    Task<IDataResult<IPagedList<Blog>>> GetListWithCategoryByPaging(int pageNumber = 1, int pageSize = 10);
    Task<IDataResult<IPagedList<Blog>>> GetListWithCategoryByWriterWithPagingAsync(string userName, int take, int page);
    ...
    Task<IDataResult<IPagedList<BlogCategoryandCommentCountDto>>> GetBlogListByMainPage(GetBlogModel getBlogModel);
